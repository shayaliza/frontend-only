...
function Notification({isOpen}) {
  return (
    <div className="">
    <div className={`${isOpen ? "block" : "hidden"} lg:flex flex-col space-y-4 sm:space-y-8 p-4 sm:p-6 bg-black lg:w-96 overflow-auto`}>
      <section className="flex items-center space-x-4 p-3 sm:p-4 bg-gray-800 rounded-lg shadow-sm text-white">
        <img src={android} alt="Android" className="w-8 h-8 sm:w-12 sm:h-12" />
        <p className="text-sm sm:text-lg font-semibold">Datasnap for Android is here!</p>
      </section>
      
      <section className="bg-gray-800 p-3 sm:p-4 rounded-lg shadow-sm text-white">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <p className="text-base sm:text-lg font-semibold">Trending Articles</p>
          <img src={trendingIcon} alt="Trending" className="w-5 h-5 sm:w-6 sm:h-6" />
        </div>
        <div className="space-y-4 sm:space-y-6">
          {trendingArticles.map((article, index) => (
            <div key={index} className="flex items-start space-x-3 sm:space-x-4">
              <img src={article.image} alt="Article" className="w-10 h-10 sm:w-12 sm:h-12 rounded-full" />
              <div>
                <h4 className="text-sm sm:text-md font-semibold">{article.title}</h4>
                ...
                <div className="flex space-x-3 sm:space-x-4 text-xs sm:text-sm mt-1 sm:mt-2">
                  <span className="flex items-center">
                    <img src={likeIcon} alt="Likes" className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                    {article.likes}
                  </span>
                  <span className="flex items-center">
                    <img src={commentIcon} alt="Comments" className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                    {article.comments}
                  </span>
                </div>
              </div>
          ...
          <p className="text-blue-500 cursor-pointer text-sm sm:text-base">Show more</p>
        </div>
      </section>

      <section className="bg-gray-800 p-3 sm:p-4 rounded-lg shadow-sm text-white">
        <p className="text-base sm:text-lg font-semibold mb-3 sm:mb-4">Recently Accessed</p>
        <div className="space-y-4 sm:space-y-6">
