...
import { createWorkSpace } from "../fetching/organization/organization";
import { useDispatch } from "react-redux";
import { useSelector } from "react-redux";
import { setWorkspaceId } from "../features/organization/workspaceSlice";
...
function Seventh() {
  ...
  const organizationId = useSelector((state) => state.workspace.organizationId);

  const handleClick = async () => {
    alert(name, description);

    const res = await createWorkSpace(name, description, organizationId);
    if (res.data) {
      console.log(res.data);
      dispatch(setWorkspaceId(res.data.id));
      navigate("/organization/eight");
    }
  };
