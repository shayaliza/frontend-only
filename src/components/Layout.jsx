    ...
    <div className="flex overflow-hidden">
      ...
      <div className={`flex flex-col w-full ${isPanelOpen ? 'blur' : ''}`}>
        ...
        <div className={`lg:ml-56 mt-20 overflow-y-auto flex-grow z-0`}>
          <Outlet context={{ toggleAddCoursePopup, toggleAddCareerPathPopup }} />
        </div>
      </div>
      <CoursePopup isOpen={isAddCoursePopup} togglePopup={toggleAddCoursePopup} />
      <CareerPathPopup isOpen={isAddCareerPathPopup} togglePopup={toggleAddCareerPathPopup} /> 
    </div>
  );
}
